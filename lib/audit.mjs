 ...
 * @property {string[]} missingPaths      declared paths matching nothing at all
 * @property {string[]} untrackedPaths    declared paths present but not committed
...
/**
 * Whether a pathspec matches a file that exists but git does not track yet.
 *
 * `--exclude-standard` is not optional: without it, ignored files count as
 * present, so a glob pointing at build output would look like work pending a
 * commit forever instead of like the wrong glob it is (BR-9).
 */
function matchesUntrackedFiles(root, glob) {
  try {
    return git(root, ['ls-files', '--others', '--exclude-standard', '--', `:(glob)${glob}`]).trim() !== '';
  } catch {
    return false;
  }
}
...
export function audit(root) {
  ...
  for (const { capability, data } of entries) {
    ...
    // Selected by workflow state, never by the presence of an attestation. After a
    // migration an entry is verified by the workflow while carrying no `verified[]`
    // at all, and keying on the attestation would silently stop auditing exactly
    // those entries - drift detection would go quiet across the whole bundle.
    if (String(data.verification_state ?? '') !== 'verified') {
      result.note = `${data.verification_state || 'no verification_state'} - already surfaced by okf check`;
      results.push(result);
      continue;
    }
    ...
    for (const glob of globs) {
      // Three states, not two. Verification precedes the commit that introduces a
      // feature's files, so every new capability spends a moment with declared
      // paths that exist and are untracked - calling that a vanished path teaches
      // people to ignore the report (BR-9).
      if (!matchesTrackedFiles(root, glob)) {
        if (matchesUntrackedFiles(root, glob)) result.untrackedPaths.push(glob);
        else result.missingPaths.push(glob);
      }
