...
/**
 * Inline code spans, so a document can name template text while explaining it.
 * A rule that makes its own documentation illegal is a rule that gets worked
 * around rather than obeyed (BR-4).
 *
 * Line-scoped on purpose. An unbalanced backtick then has nothing to pair with,
 * so it cannot swallow the rest of the file and take real residue with it -
 * which is also why callers strip fenced blocks first, before a fence's own
 * backticks are here to be paired.
 *
 * The span becomes a space rather than nothing: two adjacent spans must not
 * fuse into a token neither of them contained.
 */
...
/**
 * Template residue: text present because a template put it there and nobody
 * replaced it, as opposed to text an author wrote.
 *
 * `archiveMode` is `null` for a bundle file, which is a finished artifact by
 * construction and has no in-flight state, and `true`/`false` for a change
 * artifact, which legitimately holds a half-written section while the change is
 * being written (BR-5). `hardensAtArchive` reads a null as the warning side,
 * which is what keeps the instruction comment on a bundle file a warning.
 */
function checkHygiene(where, text, report, { archiveMode = null } = {}) {
  const clean = stripCodeSpans(stripFences(stripComments(text)));
  const residue =
    archiveMode === null ? (w, m) => report.error(w, m) : hardensAtArchive(report, archiveMode);
  const shipped = hardensAtArchive(report, archiveMode);
  ...
  if (bareBullets) {
    residue(where, `${bareBullets} empty list item(s) left from the template`);
  }

  // Comments survive here on purpose - `stripComments` would delete the very
  // thing this looks for - but fences and spans do not, or a document could not
  // name the marker while explaining what it recognises (BR-2).
  if (/HOW TO USE THIS TEMPLATE/.test(stripCodeSpans(stripFences(text)))) {
    shipped(where, 'still carries the template instruction comment - delete it once the entry has content');
  }
}
...
/**
 * Every markdown artifact of one change, scanned on the same terms as a bundle
 * file (BR-1). The two are produced from templates of the same kind and read by
 * the same people; applying the rule to one and not the other is an accident of
 * where the check was first written.
 *
 * `openspec/changes/archive/` is never reached - `activeChangeIds` does not
 * yield it, and this walk declines to descend into it a second time. An archived
 * change was archived under the rules of its time, and re-reporting it produces
 * a backlog nobody chose against a directory nobody will edit.
 */
function checkChangeHygiene(root, dir, report, { archiveMode }) {
  if (!exists(dir)) return;
  ...
  const walk = (d) => {
    for (const e of fs.readdirSync(d, { withFileTypes: true })) {
      ...
      if (e.isDirectory()) {
        if (e.name !== 'archive') walk(abs);
        continue;
      }
      if (!e.name.endsWith('.md')) continue;
      checkHygiene(rel(root, abs), read(abs), report, { archiveMode });
    }
  };
  walk(dir);
}
...
function checkChange(root, changeId, report, { archiveMode = false } = {}) {
  ...
  // Before the okf-link early return: an artifact's hygiene does not depend on
  // whether the change linked an OKF entry.
  checkChangeHygiene(root, dir, report, { archiveMode });
  ...
  const { rows, linked, noDomain, unresolved } = state;
  ...
  for (const u of unresolved) {
    ...
    } else if (u.kind === 'missing-target') {
      report.error(where, `"${u.capability}" has no OKF file and no "no domain knowledge" reason`);
    } else if (u.kind === 'missing-file') {
      report.error(where, `"${u.capability}" points at ${u.target}, which does not exist on disk`);
    }
  }
  for (const n of noDomain) {
    if (n.reason.length < 10) {
      report.error(
        where,
        `"${n.capability}" is marked as having no domain knowledge without a specific reason`
      );
    }
  }
  ...
  if (exists(testPlanFile)) {
    checkTestPlan(root, testPlanFile, report, { archiveMode, linked });
  }

  // the boundary classes the matrix was asked to consider.
  const testCasesFile = at('test-cases.md');
  if (exists(testCasesFile)) {
    checkBoundaryCoverage(dir, read(testCasesFile), rel(root, testCasesFile), report);
  }
...
/**
 * The shared read of one change's OKF half. Both the archive gate and `okf next`
 * call this so they cannot disagree about what is present, linked, still pending,
 * or evidenced. Lives after `checkChange` so the hygiene allowlist guard (UT-208)
 * does not treat these artifact names as an excused-file list.
 *
 * @returns {{
 *   artifacts: { okfLink: boolean, verification: boolean },
 *   linked: { capability: string, target: string, resolved: string }[],
 *   pendingIn: { capability: string, target: string, resolved: string }[],
 *   evidenceRows: string[][],
 *   rows: string[][],
 *   noDomain: { capability: string, reason: string }[],
 *   unresolved: { capability: string, target?: string, kind: string }[],
 * }}
 */
export function readChangeState(root, changeId) {
  const dir = path.join(root, 'openspec', 'changes', changeId);
  const linkFile = path.join(dir, 'okf-link.md');
  const verificationFile = path.join(dir, 'verification.md');

  const artifacts = {
    okfLink: exists(linkFile),
    verification: exists(verificationFile),
  };

  /** @type {ReturnType<typeof readChangeState>} */
  const empty = {
    artifacts,
    linked: [],
    pendingIn: [],
  ...
  if (!artifacts.okfLink) return empty;

  const rows = okfLinkRows(read(linkFile));
  const linked = [];
  const noDomain = [];
  const unresolved = [];
  ...
  for (const cells of rows) {
    ...
    if (/^<.*>$/.test(capability) || capability === '') {
      unresolved.push({ capability, kind: 'placeholder' });
      continue;
    }

    if (/^no domain knowledge/i.test(target)) {
      const reason = target.replace(/^no domain knowledge\s*[-:]?\s*/i, '').trim();
      noDomain.push({ capability, reason });
      continue;
    }

    if (!target) {
      unresolved.push({ capability, kind: 'missing-target' });
      continue;
    }
    const resolved = path.join(root, target);
    if (!exists(resolved)) {
      unresolved.push({ capability, target, kind: 'missing-file' });
      continue;
    }
  ...
  for (const entry of linked) {
    const { data } = parseFrontmatter(read(entry.resolved));
    if (!data) continue;
    if (asList(data.pending_changes).includes(changeId)) pendingIn.push(entry);
  }

  const evidenceRows = artifacts.verification
    ? tableUnder(read(verificationFile), /Rule Evidence/i).filter((c) => !isBlankRow(c))
    : [];

  return { artifacts, linked, pendingIn, evidenceRows, rows, noDomain, unresolved };
}
...
function initialStatusColumn(headerCells) {
  const i = headerCells.findIndex((c) => /^initial\s+status$/i.test(c));
  return i === -1 ? null : i;
}

/**
 * Which column holds this table's CURRENT status, as opposed to the one it held
 * before implementation started.
 *
 * A table with both columns answers with `Status`; one with only
 * `Initial Status` - the Pre-Implementation Unit Tests table - answers with
 * that, because where those tests started is all they ever record. Reading a
 * historical status as a live one turns an honest record of a promoted skeleton
 * into a false report of an untested requirement.
 *
 * @param {string[]} headerCells
 * @returns {number | null} the column index, or null if the table has none
 */
function liveStatusColumn(headerCells) {
  const current = headerCells.findIndex((c) => /^status$/i.test(c));
  if (current !== -1) return current;
  return initialStatusColumn(headerCells);
}

const columnIndex = (header, re) => {
  const i = header.findIndex((c) => re.test(c));
  return i === -1 ? null : i;
};

/**
 * The escalation every record the test-plan requires shares: a warning while the
 * change is in flight, an error at the archive boundary.
 *
 * A plan is written before implementation, when an assertion message and an
 * initial status are not knowable yet, so an error would fire at a plan doing
 * exactly what the gate asks. But `--archive` is the last moment the kit gets to
 * insist, and a MUST that is only ever a warning can be archived unmet.
 *
 * @param {{error: Function, warn: Function}} report
 * @param {boolean} archiveMode
 * @returns {(where: string, message: string) => void}
 */
function hardensAtArchive(report, archiveMode) {
  return archiveMode
    ? (where, message) => report.error(where, message)
    : (where, message) => report.warn(where, message);
}

/**
 * How long a stated reason has to be before it distinguishes anything. Shared by
 * the two places that ask for one, because one arbitrary constant used twice is a
 * convention and two different ones are a bug waiting to be reported.
 */
const REASON_MIN = 10;

/**
 * A status cell split into the word and whatever explains it.
 *
 * Two of the four statuses have to answer for themselves - `failing` with the
 * assertion it failed on, `passing` in the initial column with the reason it was
 * green before its implementation existed - and both are written the same way:
 * `word: reason`.
 *
 * @param {string} raw
 * @returns {{word: string, reason: string}}
 */
function explainedStatus(raw) {
  const text = String(raw).trim();
  const word = text.split(/[\s:]/)[0].toLowerCase();
  const colon = text.indexOf(':');
  return { word, reason: colon === -1 ? '' : text.slice(colon + 1).trim() };
}

/**
 * Where a table records what would make each of its tests fail, by header name
 * like every other column lookup in this file.
 *
 * @param {string[]} header
 * @returns {number | null}
 */
function falsifierColumn(header) {
  return columnIndex(header, /^falsified\s+by$/i);
}

/**
 * Which of the two admissible grounds a Test Changes row stands on.
 *
 * A pre-written test may be edited on exactly two grounds: an upstream rule or
 * spec changed first, or the test had a mechanical defect. A row therefore
 * answers with a citation or with a named defect - the same reason-or-path shape
 * the Decision Promotion table and the Not Applicable declaration already use,
 * and for the same reason: silence is the one answer that must not pass.
 *
 * @param {string[]} cells    the row
 * @param {string[]} header   its table header
 * @returns {{kind: 'citation' | 'defect' | 'none', value: string}}
 */
function testChangeGround(cells, header) {
  const cite = columnIndex(header, /rule.*spec|spec change/i);
  const ground = columnIndex(header, /^(ground|reason)$/i);

  const cited = cite === null ? '' : (cells[cite] || '').replace(/[`\s]/g, '');
  if (cited && cited !== '-') return { kind: 'citation', value: cited };

  const stated = ground === null ? '' : (cells[ground] || '').trim();
  const defect = /^mechanical defect\s*[:,.\-–—]?\s*(.*)$/i.exec(stated);
  if (defect && defect[1].trim().length >= REASON_MIN) return { kind: 'defect', value: defect[1].trim() };

  return { kind: 'none', value: stated };
}

/**
 * Every row of the Test Changes table answers for itself: which test, and on
 * which ground. A citation is checked for resolving, never for being apt - that
 * a row cites BR-3 is mechanical, that BR-3 is really why the test moved is a
 * review question.
 */
function checkTestChanges(root, where, text, linked, report) {
  const heading = /Test Changes After Implementation Started/i;
  const rows = tableUnder(text, heading);
  if (!rows.length) return;

  const header = tableHeaderUnder(text, heading);
  const testCol = columnIndex(header, /^test$/i);

  for (const cells of rows) {
    if (isBlankRow(cells)) continue;
    const named = testCol === null ? '' : (cells[testCol] || '').trim();
    const label = named || (cells[0] || '').trim() || 'a row';

    if (!named) {
      report.error(where, `a Test Changes row names no test - "${cells.join(' | ')}" records nothing`);
    }

    const ground = testChangeGround(cells, header);
    if (ground.kind === 'none') {
      report.error(
        where,
        `"${label}" states no ground for changing a pre-written test - cite the BR-n or spec that ` +
          'changed first, or declare the mechanical defect and name it'
      );
      continue;
    }
    if (ground.kind !== 'citation') continue;

    const ruleIds = ground.value.match(/BR-\d+/g);
    if (ruleIds) {
      const known = new Set();
      for (const entry of linked) for (const id of collectRuleIds(read(entry.resolved))) known.add(id);
      for (const id of ruleIds) {
        if (!known.has(id)) {
          report.error(
            where,
            `"${label}" cites ${id}, which no OKF entry linked by this change carries - ` +
              'a citation that resolves to nothing is not a ground'
          );
        }
      }
      continue;
    ...
    if (/^[\w.\-/]+\.\w{1,6}$/.test(ground.value) && !exists(path.join(root, ground.value))) {
      report.error(
        where,
        `"${label}" cites "${ground.value}", which does not exist - a dangling citation is not a ground`
      );
    }
  }
}

function checkTestPlan(root, file, report, { archiveMode, linked = [] }) {
  const where = rel(root, file);
  const text = read(file);
  const clean = stripComments(text);

  // A test changed after implementation started answers for why, whether or not
  // the rest of the plan is exempt: this table exists only when someone edited a
  // pre-written test, and that is precisely when the reason matters.
  checkTestChanges(root, where, text, linked, report);
  ...
  let falsifier = null;
  const pendingRows = [];
  const hardens = hardensAtArchive(report, archiveMode);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!header) {
      header = cells;
      // The falsifier is asked of the tests BR-3 requires to reach the red state.
      // Integration and E2E rows legitimately start as skeletons, before the
      // assertion exists to have a falsifier.
      unitTable = /Pre-Implementation Unit Tests/i.test(heading);
      falsifier = unitTable ? falsifierColumn(header) : null;
      if (unitTable && falsifier === null) {
        hardens(
          where,
          'the Pre-Implementation Unit Tests table has no "Falsified By" column - a test nobody ' +
            'can name a production change for cannot report a disagreement with one'
        );
      }
      continue;
    }
    ...
    const live = liveStatusColumn(header);
    const initial = initialStatusColumn(header);
    const rowId = (cells[0] || '').trim();

    if (unitTable && falsifier !== null && !(cells[falsifier] || '').trim()) {
      hardens(where, `"${rowId}" names no falsifier - say which production change would make this test fail`);
    }

    for (const idx of statusColumnIndexes(header)) {
      const raw = (cells[idx] || '').trim();
      if (!raw) continue;
      const { word, reason } = explainedStatus(raw);
      if (!TEST_STATUS.includes(word)) {
        report.error(where, `unknown test status "${raw}" - use ${TEST_STATUS.join(' | ')}`);
        continue;
      }
      if (word === 'failing' && !reason) {
        hardens(
          where,
          `"${raw}" records no assertion message - Initial Status should read like "failing: expected 403, got 200"`
        );
      }
      // Green before its implementation existed means one of two things, and the
      // bare word cannot tell them apart: the rule already held and this test
      // locks it, or the test asserts nothing this change introduces.
      if (unitTable && idx === initial && word === 'passing' && reason.length < REASON_MIN) {
        hardens(
          where,
          `"${rowId}" was green before its implementation existed and does not say why - write ` +
            '`passing: <reason>`, or the row reads as a test that asserts nothing this change introduces'
        );
      }
      // Only the live column can describe a gap. A historical `skeleton` that
      // ended `passing` is the record working, not a requirement slipping through.
      if (idx === live && (word === 'skeleton' || word === 'planned')) {
        pendingRows.push({ id: (cells[0] || '').trim(), word });
      }
    }

    // A row that says where it ended but not where it started is
    // indistinguishable from one whose test was written after the code.
    if (live !== null && live !== initial && (cells[live] || '').trim()) {
      if (initial === null || !(cells[initial] || '').trim()) {
        report.warn(
          where,
          `"${(cells[0] || '').trim()}" records no status from before implementation - fill Initial Status, ` +
            'or this plan cannot show the test predates the code it guards'
        );
      }
    }
  }
  ...
  if (archiveMode && pendingRows.length) {
    ...
    // By header name, like every other column lookup here. A positional read is
    // correct only for the shipped column order, and reports nothing when a
    // reordered table makes it wrong.
    const gapsHeader = tableHeaderUnder(text, /Known Gaps/i);
    const idCol = columnIndex(gapsHeader, /test case id|^id$/i) ?? 0;
    const ownerCol = columnIndex(gapsHeader, /^owner$/i);
    for (const row of pendingRows) {
      const listed =
        ownerCol !== null &&
        gaps.some((cells) => (cells[idCol] || '').trim() === row.id && (cells[ownerCol] || '').trim() !== '');
      if (!listed) {
        report.error(
          where,
...
/**
 * Whether a test-case matrix was asked to consider anything at all (BR-13).
 *
 * This is a warning and never an error, in flight and at the archive boundary
 * alike - the one check here that does not harden. Whether the six named classes
 * were genuinely considered is not observable from the file: nothing can tell a
 * considered row from a fabricated one, and a gate that pretended otherwise
 * would produce six rows written to satisfy it. Total silence is the one state
 * that IS observable, so it is the only one reported.
 *
 * It counts rows and never classes, for the same reason - counting classes would
 * require judging what a class is, which is the judgement this refuses to make.
 *
 * The specs are what make the silence mean something. A change with no scenarios
 * has nothing to have found a boundary in yet, and warning at it would fire at a
 * matrix doing exactly what the gate asks.
 *
 * @param {string} changeDir       absolute path to the change directory
 * @param {string} testCasesText   the raw text of its `test-cases.md`
 * @param {string} where           repo-relative path, for the finding
 * @param {{error: Function, warn: Function}} report
 * @returns {void}
 */
function checkBoundaryCoverage(changeDir, testCasesText, where, report) {
  if (!specScenarioCount(changeDir)) return;

  const heading = /Negative And Boundary Cases/i;
  // A blank row is the template's seed with nothing written into it, which is
  // the state this check is about - counting it as a row makes the warning
  // unreachable from the shipped template.
  const rows = tableUnder(testCasesText, heading).filter((cells) => !isBlankRow(cells));
  if (rows.length) return;

  report.warn(
    where,
    'the Negative And Boundary Cases table has no rows while the specs carry scenarios - an empty ' +
      'table records that nobody was asked to think of a boundary class, not that there are none'
  );
}

/** How many `#### Scenario:` headings this change's delta specs carry. */
function specScenarioCount(changeDir) {
  let count = 0;
  const walk = (dir) => {
    if (!exists(dir)) return;
    for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      else if (e.name.endsWith('.md')) count += (read(p).match(/^#{1,6}\s+Scenario:/gm) || []).length;
    }
  };
  walk(path.join(changeDir, 'specs'));
  return count;
}
...
/**
 * The two static analysis categories every change answers for (BR-10).
 *
 * Categories, not tools: the Check column says `Lint`, the Command column says
 * whatever the project runs. Keying the requirement on the tool would make the
 * gate wrong in every ecosystem but one, and would read a project's move from
 * `eslint` to `biome` as a missing row.
 */
const REQUIRED_CHECKS = ['Lint', 'Typecheck'];

/**
 * Words that occupy a Result cell without answering it. `-` and the empty cell
 * are handled by `cellEmpty`; these are the ones that look filled in.
 */
const NON_RESULTS = /^(not run|pending|todo|tbd|n\/a|na|see below|-+)$/i;

/** `Type check`, `type-check` and `Typecheck` are the same answer. */
const foldCheckName = (s) => s.replace(/[\s_-]+/g, '').toLowerCase();

/**
 * The static analysis gate (BR-9..BR-12).
 *
 * Change-scoped like decision promotion, and for the same reason: a change with
 * no domain knowledge still compiles or fails to. Reports through
 * `hardensAtArchive` because a result is not knowable before the code that
 * produces it is written.
 *
 * The Command column is deliberately not gated. A row nobody filled in has no
 * result either, and the result is what this reads - so the untouched template
 * row is still caught, without a second placeholder implementation to drift.
 */
function checkStaticAnalysis(verificationText, where, report, { archiveMode }) {
  const say = hardensAtArchive(report, archiveMode);
  const rows = tableUnder(verificationText, /Static Analysis/i).filter((c) => !isBlankRow(c));

  if (!rows.length) {
    say(
      where,
      'the Static Analysis table is empty or missing - a green test suite is not evidence that the code lints or ' +
        `type-checks, so record ${REQUIRED_CHECKS.join(' and ')}, or state why there is nothing to record`
    );
    return;
  }
  ...
  for (const cells of rows) {
    const name = (cells[0] || '').replace(/`/g, '').trim();
    const result = (cells[2] || '').replace(/`/g, '').trim();
    if (!name) {
      report.error(where, 'a Static Analysis row has no Check name');
      continue;
    }
    seen.add(foldCheckName(name));

    if (cellEmpty(result) || NON_RESULTS.test(result)) {
      say(where, `Static Analysis row "${name}" has no result - "${result || '(empty)'}" records that nobody looked`);
      continue;
    }

    const declared = notApplicableDeclaration(`- ${result}`);
    if (declared && declared.reason.length < REASON_MIN) {
      say(
        where,
        `Static Analysis row "${name}" is discharged as not applicable with no reason - ` +
          'name what makes it true, as every other escape hatch here does'
      );
    }
  }

  for (const required of REQUIRED_CHECKS) {
    if (!seen.has(foldCheckName(required))) {
      say(
        where,
        `the Static Analysis table has no ${required} row - a project that runs none discharges the row with a ` +
          'stated reason, because a deleted row reads the same as an oversight'
      );
    }
  }
}
...
function checkVerification(root, changeDir, file, linked, changeId, report, { archiveMode }) {
  ...
  // Change-scoped, and deliberately outside the archiveMode guard below: this
  // gate reports at both levels through `hardensAtArchive`, because a result is
  // not knowable before the code that produces it is written (BR-9).
  checkStaticAnalysis(text, where, report, { archiveMode });
